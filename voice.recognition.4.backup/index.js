...
const commands = [
    'yes', 'no', 'left', 'right', 'forward', 'back', 'stop', 'faster', 'slower', 'wallace'
...
const createFeaturesInputArray = () => {
    let currFeaturesInputArrayIndex = 0;
    const featuresInputArray = new Array(MAX_NEURAL_NETWORK_INPUT_FEATURES).fill(0);
    featuresInputArray[currFeaturesInputArrayIndex] = phraseTotalMs;
    currFeaturesInputArrayIndex++;
    phrasePartsArray.forEach((part,i) => {
        if (part > 0) {
            featuresInputArray[currFeaturesInputArrayIndex] = part;
        }
        currFeaturesInputArrayIndex++;
    })
    posVolumeArray.forEach((volume,i) => {
        if (volume > 128) {
            featuresInputArray[currFeaturesInputArrayIndex] = volume;
        }
        currFeaturesInputArrayIndex++;
    })
    negVolumeArray.forEach((volume,i) => {
        if (volume < 127) {
            featuresInputArray[currFeaturesInputArrayIndex] = volume;
        }
        currFeaturesInputArrayIndex++;
...
const _trainUntilPredict = () => {
    ...
    setTimeout(()=> {
        ...
        if (percentCorrect > .90 || numTrainingTries > 60) {
...
const analyzeFrequencyBytes = (bytes) => {
    ...
    freqInfo.push(bytes.subarray(block * 8, block * 9).reduce(sumArrayFunc));
    freqInfo.push(bytes.subarray(block * 9, block * 10).reduce(sumArrayFunc));
    freqInfo.push(bytes.subarray(block * 10, block * 11).reduce(sumArrayFunc));
    freqInfo.push(bytes.subarray(block * 11, block * 12).reduce(sumArrayFunc));
    freqInfo.push(bytes.subarray(block * 12, block * 13).reduce(sumArrayFunc));
    freqInfo.push(bytes.subarray(block * 13, block * 14).reduce(sumArrayFunc));
    freqInfo.push(bytes.subarray(block * 14, block * 15).reduce(sumArrayFunc));
    return freqInfo;
}
...
const analyzeTimeDomainBytes = (bytes, frequencies) => {
    ...
    if (min < (127 - minSoundDelta) || max > (128 + minSoundDelta)) {
        ...
        if (!numPhrasesIncremented && silenceMs > silenceMsParam) {
            ...
            console.log('VolArr:', currVolumeArrayIndex, ' ', posVolumeArray.length);
            showVolumeArrayVisually(posVolumeArray);
